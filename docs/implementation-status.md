...
- Known issues: real Dify smoke is `NOT RUN` without complete ignored local configuration; real PostgreSQL migration/restart/persistence is `NOT RUN` without an authorized dedicated database. No credential from the external Dify reference tree was used.
- Next step: the provider-neutral baseline is complete; external compatibility checks remain separately configured and explicitly unclaimed.

## Post-Milestone Extension — Browser Conversation Resume

- Status: completed within the local and provider-simulated acceptance scope; real Dify smoke externally unperformed
- Completed: provider-neutral resume contracts and ports; AES-256-GCM opaque tokens; bounded chronological Dify history retrieval; atomic in-memory/PostgreSQL restoration; Runtime/API restart recovery; browser `skein.chat.history.v1` cache; accessible desktop sidebar/mobile drawer; cached-first canonical reconciliation; read-only failure/retry behavior; sanitized restart smoke; and setup/security/limitation documentation.
- Tests: the post-review repair gate passed 31 test files / 482 tests with lint, typecheck, build, scrub and diff checks clean. Rendered browser QA passed at 1280x720 and 390x844; the 390x844 close, backdrop and new-conversation paths returned focus to the history opener, with no console errors on exercised paths.
- Design decisions: the public identity remains the Skein `sessionId`; external conversation IDs remain encrypted/private; localStorage stores only canonical messages, title, public session ID and opaque token; restore is capped at 200 Dify rows and reconstructs default context revision `0`; browser cache is capped at 400 messages per conversation; PostgreSQL remains the full durability path.
- Known issues: the real Dify restart/resume smoke is `NOT RUN` because `DIFY_BASE_URL`, `DIFY_API_KEY` and `SESSION_RESUME_SECRET` were not all available in ignored local configuration. Real PostgreSQL acceptance remains `NOT RUN` without an authorized dedicated test database. V1 recovery is Dify-only and does not reconstruct provider-invisible Skein turn, summary, audit or workflow state.
- Next step: configure ignored local Dify values to run `pnpm smoke:dify`, or configure the dedicated PostgreSQL acceptance database for full durability verification.
